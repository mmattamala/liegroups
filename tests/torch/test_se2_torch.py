...
def test_from_matrix(device):
    T_good = SE2.from_matrix(torch.eye(3).to(device))
    ...
    T_bad = SE2.from_matrix(torch.eye(3).add_(1e-3).to(device), normalize=True)
...
def test_from_matrix_batch(device):
    T_good = SE2.from_matrix(torch.eye(3).to(device).repeat(5, 1, 1))
    assert isinstance(T_good, SE2) \
...
def test_identity_batch(device):
    T = SE2.identity(5, device=device)
    assert isinstance(T, SE2) \
        and isinstance(T.rot, SO2) \
        and T.rot.mat.dim() == 3 \
...
def test_dot(device):
    ...
    pt = torch.Tensor([1, 2]).to(device)
    pth = torch.Tensor([1, 2, 1]).to(device)
...
def test_dot_batch(device):
    T1 = torch.Tensor([[0, -1, -0.5],
                       [1, 0, 0.5],
                       [0, 0, 1]]).expand(5, 3, 3).to(device)
    T2 = torch.Tensor([[0, -1, -0.5],
                       [1, 0, 0.5],
                       [0, 0, 1]]).to(device)
    T1_SE2 = SE2.from_matrix(T1)
    T2_SE2 = SE2.from_matrix(T2)
    pt1 = torch.Tensor([1, 2]).to(device)
    pt2 = torch.Tensor([4, 5]).to(device)
    pt3 = torch.Tensor([7, 8]).to(device)
    pts = torch.cat([pt1.unsqueeze(dim=0),
                     pt2.unsqueeze(dim=0),
                     pt3.unsqueeze(dim=0)], dim=0)  # 3x2
    ptsbatch = pts.unsqueeze(dim=0).expand(5, 3, 2)
    pt1h = torch.Tensor([1, 2, 1]).to(device)
    pt2h = torch.Tensor([4, 5, 1]).to(device)
    pt3h = torch.Tensor([7, 8, 1]).to(device)
...
def test_wedge_vee(device):
    xi = torch.Tensor([1, 2, 3]).to(device)
    Xi = SE2.wedge(xi)
    assert (xi == SE2.vee(Xi)).all()


def test_wedge_vee_batch(device):
    xis = torch.Tensor([[1, 2, 3], [4, 5, 6]]).to(device)
    Xis = SE2.wedge(xis)
    assert (xis == SE2.vee(Xis)).all()


def test_odot(device):
    p1 = torch.Tensor([1, 2]).to(device)
    p2 = torch.Tensor([1, 2, 1]).to(device)
    p3 = torch.Tensor([1, 2, 0]).to(device)

    odot12 = torch.cat([SE2.odot(p1), torch.zeros(3, device=device).unsqueeze_(dim=0)], dim=0)
    odot13 = torch.cat([SE2.odot(p1, directional=True),
                        torch.zeros(3, device=device).unsqueeze_(dim=0)], dim=0)
    odot2 = SE2.odot(p2)
    odot3 = SE2.odot(p3)
...
def test_exp_log(device):
    T = SE2.exp(torch.Tensor([1, 2, 3]).to(device))
...
def test_perturb(device):
    ...
    xi = torch.Tensor([0.3, 0.2, 0.1]).to(device)
    T.perturb(xi)
...
def test_normalize_batch(device):
    ...
    T.rot.mat.add_(0.1)
    assert (SE2.is_valid_matrix(T.as_matrix().cpu())
...
def test_inv(device):
    T = SE2.exp(torch.Tensor([1, 2, 3])).to(device)
    assert utils.allclose((T.dot(T.inv())).as_matrix(), torch.eye(3, device=device))
...
def test_inv_batch(device):
    ...
    assert utils.allclose(T.dot(T.inv()).as_matrix(),
                          SE2.identity(T.trans.shape[0], device=device).as_matrix())
