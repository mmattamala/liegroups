...
def test_from_matrix(device):
    T_good = SE2.from_matrix(torch.eye(3, device=device))
    ...
    T_bad = SE2.from_matrix(torch.eye(3, device=device).add_(1e-3), normalize=True)
...
def test_from_matrix_batch(device):
    T_good = SE2.from_matrix(torch.eye(3, device=device).repeat(5, 1, 1))
    assert isinstance(T_good, SE2) \
...
def test_dot_batch(device):
    T1 = torch.Tensor([[0, -1, -0.5],
                       [1, 0, 0.5],
                       [0, 0, 1]], device=device).expand(5, 3, 3)
